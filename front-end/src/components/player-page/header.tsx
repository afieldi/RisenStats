...
export default function PlayerPageHeader(playerOverviewProps: Props) {
    ...
    function updateGames() {
        if (playerOverviewProps.playerOverview) {
            ...
            UpdatePlayer(playerOverviewProps.playerOverview.overview.puuid).then(() => {
                setLoading(false);
                navigate('./');
            }, (err) => {
                alert(err);
    ...
    return (
        <Container sx={{display: 'flex', alignItems: 'flex-start'}}>
            <Box sx={{display: 'inline-flex', alignItems: 'flex-start'}}>
                <img src={`https://ddragon.leagueoflegends.com/cdn/12.19.1/img/profileicon/${playerIcon}.png`}
                ...
                <Box sx={{flexGrow: 1, pl: 2, textAlign: 'left'}}>
                    ...
                    <Hidden smDown>
                        <Typography variant="h3"
                                    className="text-overflow">{playerOverviewProps.playerOverview?.overview?.name}</Typography>
                    </Hidden>
                    {/* <br></br> */}
                    <LoadingButton variant="outlined" color="primary" onClick={updateGames}
