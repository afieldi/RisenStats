...
export default function TeamListBox(props: TeamsListProps) {
  ...
  return (
    <Box sx={{ minWidth: 280,
      ...
      <Box sx={{ p: 0.2, borderRadius: '4px 4px 0px 0px', bgcolor: theme.palette.risenBoxBg.main }}>
      ...
      <Box sx={{ p: 1, borderRadius: '0px 0px 4px 4px', bgcolor: theme.palette.risenBoxBg.main }}/>
    </Box>
  );
}
...
function TeamRow(bgColor: string, nameTypography: React.ReactNode, gamesTypography: React.ReactNode, wrTypography: React.ReactNode) {
  ...
  return (
    <Box sx={{ pt: 0.5, pb: 0.5, display: 'flex', flexDirection: 'row', justifyContent:'space-between', backgroundColor: bgColor }}>
      {nameTypography}
