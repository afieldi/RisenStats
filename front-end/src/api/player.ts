import {
    PlayerChampionStatsRequest,
    PlayerChampionStatsResponse,
    PlayerGamesResponse,
...
export async function GetPlayerChampionStats(playerPuuid: string, seasonId?: number, risenOnly?: boolean, roleId?: string): Promise<PlayerChampionStatsResponse> {
    ...
    return await MakeBackendCall<PlayerChampionStatsRequest>(`/api/player/champions/by-puuid/${playerPuuid}`, "POST", {
        seasonId,
        risenOnly,
        roleId: role
    }) as PlayerChampionStatsResponse;
}
...
export async function GetPlayerStats(playerPuuid: string, seasonId?: number, roleId?: string, risenOnly?: boolean): Promise<GetPlayerStatsResponse> {
    return await MakeBackendCall<GetPlayerStatsRequest>(`/api/stats/player/by-puuid/${playerPuuid}`, "POST", {
        seasonId,
        roleId,
        risenOnly
