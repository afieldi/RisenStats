...
async function getGuildUser(userId: string) {
  return fetch(`https://discord.com/api/guilds/${process.env.DISCORD_SERVER_ID}/members/${userId}`, {
    method: "GET",
    headers: {
...
export async function DoAuth(code: string, host: string): Promise<AuthUser> {
  const redirect_uri = (process.env.NODE_ENV === "production" ? `https://${host}` : 'http://localhost:4000') + '/api/auth/callback';
  ...
  return oauth.tokenRequest({
    ...
    return oauth.getUser(userAuth.access_token).then(async user => {
